...
for DB in "$DB_NAME" "$DB_TEST"; do
  if ! sudo mysql -e "SHOW DATABASES LIKE '$DB';" | grep -q "$DB"; then
...
sudo mysql -e "GRANT ALL PRIVILEGES ON $DB_TEST.* TO '$DB_USER'@'localhost';"
...
if [ -f "$SCHEMA_FILE" ]; then
  if ! mysql -u"$DB_USER" -p"$DB_PASS" "$DB_NAME" -e "SHOW TABLES LIKE 'commit';" | grep -q "commit"; then
    ...
    sed "s/$DB_NAME/$DB_TEST/g" "$SCHEMA_FILE" | sudo mysql "$DB_TEST"
  else
