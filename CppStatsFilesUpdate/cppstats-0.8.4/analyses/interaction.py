...
#NUOVA MODIFICA PER SOSTITUIRE PSTAT
import numpy as np
...
def _countNestedIfdefs(root):
    ...
    if (len(cnlist) > 0):
        ...
        nnimean = np.mean(nnitmp) if nnitmp else 0
    else:
        nnimax = 0
        nnimean = 0
    if (len(cnlist) > 1): nnistd = np.std(cnlist, ddof=1)
    else: nnistd = 0
...
def _getScatteringTanglingDegrees(sigs, defines):
    ...
    if (len(scat) > 1): sdegstd = np.std(scat, ddof=1)
    else: sdegstd = 0

    if (len(tang)): tdegmean = np.mean(tang)
    else: tdegmean = 0
    if (len(tang) > 1): tdegstd = np.std(tang, ddof=1)
    else: tdegstd = 0
